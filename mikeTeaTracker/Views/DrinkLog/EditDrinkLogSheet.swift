...
struct EditDrinkLogSheet: View {
    ...
    @Query private var allDrinkLogs: [DrinkLog]
    ...
    private var drinkLog: DrinkLog? {
        allDrinkLogs.first { $0.id == drinkLogId }
    }
    ...
    var body: some View {
        NavigationStack {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            HStack {
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    Text(languageManager.isEnglish ? log.brandName : log.brandNameZH)
                                        .font(.system(size: 14))
            ...
            .onAppear {
                    // Initialize state when log is loaded
                    selectedSize = log.size
                    selectedSugarLevel = log.sugarLevel
                    selectedIce = log.iceLevel
                    priceText = log.price.map { String(format: "%.2f", $0) } ?? ""
                    calorieText = String(format: "%.0f", log.calories)
                    sugarText = String(format: "%.1f", log.sugarGrams)
                }
            } else {
                ProgressView()
                    .navigationTitle(String(localized: "edit_drink"))
    ...
    private func saveChanges() {
        guard let log = drinkLog else { return }
        
        // Update the drinkLog with new values
        log.size = selectedSize
        log.sugarLevel = selectedSugarLevel
        log.iceLevel = selectedIce
